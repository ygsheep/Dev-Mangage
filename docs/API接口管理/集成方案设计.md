...
class URLPatternGenerator {
  ...
  private generateCreateParameters(
    table: DatabaseTable,
    ...
    return table.fields
      .filter(field => {
        ...
        if (['created_at', 'updated_at', 'createdAt', 'updatedAt'].includes(field.name))
          return false
      ...
      .map(field => {
        ...
        return {
          name:
            fieldRule.apiName ||
            this.transformFieldName(field.name, config.tableConfig.namingStyle),
  ...
  private generateListParameters(
    table: DatabaseTable,
    config: CRUDGenerationRules
  ): APIParameter[] {
    ...
    if (config.generationOptions.includeSearch) {
      ...
      table.fields
        .filter(field => {
          const fieldRule = config.fieldRules[field.name]
          return (!fieldRule || fieldRule.apiVisible !== false) && this.isFilterableField(field)
        })
        .forEach(field => {
          const fieldRule = config.fieldRules[field.name] || {}
          const apiName =
            fieldRule.apiName || this.transformFieldName(field.name, config.tableConfig.namingStyle)
  ...
  private enhanceTypeWithSemantics(field: DatabaseField, apiType: APITypeDefinition): void {
    ...
    if (fieldName === 'status' || comment.includes('状态')) {
      if (field.enumValues && field.enumValues.length > 0) {
        ...
        apiType.enumDescriptions = field.enumValues.reduce(
          (acc, ev) => {
  ...
  generateRelationshipEndpoints(
    ...
    relationships.forEach(relationship => {
      ...
      if (relationship.type === 'ONE_TO_MANY') {
        ...
        endpoints.push(
          ...this.generateManyToManyEndpoints(fromTable, toTable, relationship, config)
        )
      }
    })
...
class ModelChangeDetector {
  ...
  private compareFields(oldField: DatabaseField, newField: DatabaseField): FieldChange[] {
    ...
    if (oldField.length !== newField.length) {
      const impactLevel =
        newField.length && oldField.length && newField.length < oldField.length ? 'HIGH' : 'LOW'
      changes.push({
  ...
  private isBreakingTypeChange(oldType: string, newType: string): boolean {
    ...
    return breakingChanges.some(
      ([from, to]) => oldType.toUpperCase().includes(from) && newType.toUpperCase().includes(to)
    )
  }
}
```
...
class SyncDecisionEngine {
  ...
  private analyzeAffectedItems(
    ...
    if ('tableId' in changes) {
      ...
      // 找到关联的API端点
      const relatedEndpoints = relatedItems.filter(
        item => item.type === 'API_ENDPOINT' && item.relatedTableId === tableId
      )
      ...
      // 找到关联的表关系
      const relatedRelationships = relatedItems.filter(
        item =>
  ...
  private async detectAPIModelConflicts(
    ...
    for (const change of apiChange.changes) {
      if (change.type === 'PARAMETER_ADDED' && change.newParameter?.required) {
        // 检查新增必填参数是否有对应的字段
        const relatedField = await this.findRelatedField(change.newParameter.relatedFieldId)
...
const IntegrationStatusPanel: React.FC<IntegrationStatusPanelProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* 同步状态概览 */}
      <div className="bg-bg-paper rounded-lg border p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-medium text-text-primary">同步状态</h3>
          <Button
            onClick={onSyncNow}
            size="sm"
      ...
      {/* 冲突列表 */}
      {conflicts.length > 0 && (
        <div className="bg-bg-paper rounded-lg border">
      ...
      <div className="bg-bg-paper rounded-lg border">
        <div className="p-6 border-b">
          <h3 className="text-lg font-medium text-text-primary">同步历史</h3>
        </div>
        <div className="p-6">
...
class IntegrationEventBus extends EventEmitter {
  ...
  // 类型安全的事件订阅
  on<K extends keyof IntegrationEventMap>(event: K, listener: IntegrationEventMap[K]): this {
