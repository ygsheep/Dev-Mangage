  ...
  const filteredRequests = useMemo(() => {
    return networkRequests.filter(request => {
      ...
      const statusMatch = filterStatus === 'all' ||
        (filterStatus === 'success' && (!request.error && (!request.status || request.status < 400))) ||
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-3 bg-gray-50 border-b border-gray-200">
        ...
        <div className="flex items-center space-x-4 text-xs text-gray-500">
          ...
          <span>成功: {networkRequests.filter(r => !r.error && (!r.status || r.status < 400)).length}</span>
          <span>错误: {networkRequests.filter(r => r.error || (r.status && r.status >= 400)).length}</span>
          <span>平均耗时: {
            networkRequests.length > 0 
              ? formatDuration(networkRequests.reduce((sum, r) => sum + (r.duration || 0), 0) / networkRequests.length)
              : 'N/A'
          }</span>
        </div>
      </div>
            ...
            {filteredRequests.map((request) => (
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <span className={`px-1 py-0.5 rounded text-xs font-medium ${getStatusColor(request)}`}>
                ...
                {request.error && (
                  <div className="mt-1 ml-20 text-red-600 text-xs">
                    错误: {request.error}
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    ...
                    <div className={`mt-1 px-2 py-1 rounded text-center ${getMethodColor(selectedRequest.method)}`}>
                      {selectedRequest.method}
                    </div>
                  </div>
                  <div>
                    <label className="text-gray-500 font-medium">状态:</label>
                    <div className={`mt-1 px-2 py-1 rounded text-center ${getStatusColor(selectedRequest)}`}>
