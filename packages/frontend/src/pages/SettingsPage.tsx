...
const SettingsPage: React.FC = () => {
  ...
  const generateURLConfig = () => {
    return {
      mcpServers: {
        "dev-manage-mcp-url": {
          url: `http://${serverHost}:${serverPort}/mcp`,
          transport: "http"
        }
      }
    }
  ...
  const generateWebSocketConfig = () => {
    return {
      mcpServers: {
        "dev-manage-mcp-ws": {
          url: `ws://${serverHost}:${serverPort}/mcp/ws`,
          transport: "websocket"
        }
      }
    }
  ...
  // 复制配置到剪贴板
  const copyConfig = (configType: 'development' | 'production' | 'http' | 'websocket') => {
    let config
    switch (configType) {
      case 'development':
  ...
  // 下载配置文件
  const downloadConfig = (configType: 'development' | 'production' | 'http' | 'websocket') => {
    let config
    switch (configType) {
      case 'development':
        config = generateMCPConfig()
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
              <h3 className="text-lg font-semibold text-gray-900 mb-4">服务器地址配置</h3>
            ...
            <div className="bg-white rounded-lg shadow border border-gray-200 p-6">
              ...
              <p className="text-gray-600 mb-6">
                将以下配置添加到您的 Claude Desktop 或其他 MCP 客户端配置文件中。选择适合您环境的配置：
              </p>
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-sm text-yellow-800">
                    ...
                    <ol className="space-y-1 text-xs list-decimal list-inside">
                      ...
                      <li>复制对应环境的配置到 Claude Desktop 配置文件</li>
                      <li>重启 Claude Desktop 应用</li>
                      <li>在对话中使用 MCP 工具进行 API 搜索</li>
                    </ol>
                  </div>
                </div>
              </div>
