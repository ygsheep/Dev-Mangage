...
import { mcpConfig, getMCPUrls } from './mcpConfig'
...
describe('MCPConfig', () => {
  ...
  describe('MCP 工具 URL 生成', () => {
    test('应该生成正确的工具调用 URL', () => {
      ...
      expect(url).toBe('http://localhost:3001/api/v1/mcp/tools/search_projects')
    })

    test('应该生成正确的状态 URL', () => {
      const url = mcpConfig.getMCPStatusUrl()
      expect(url).toBe('http://localhost:3001/api/v1/mcp/status')
    })

    test('应该生成正确的日志 URL', () => {
      const url = mcpConfig.getMCPLogsUrl()
      expect(url).toBe('http://localhost:3001/api/v1/mcp/logs')
    })

    test('应该生成正确的启动 URL', () => {
      const url = mcpConfig.getMCPStartUrl()
      expect(url).toBe('http://localhost:3001/api/v1/mcp/start')
    })

    test('应该生成正确的停止 URL', () => {
      const url = mcpConfig.getMCPStopUrl()
      expect(url).toBe('http://localhost:3001/api/v1/mcp/stop')
    })

    test('应该生成正确的连接测试 URL', () => {
      const url = mcpConfig.getMCPPingUrl()
      expect(url).toBe('http://localhost:3001/api/v1/mcp/ping')
    })
  })
  ...
  describe('流式 URL 生成', () => {
    test('应该生成正确的状态流 URL', () => {
      ...
      expect(url).toBe('http://localhost:3001/api/v1/mcp/status/stream')
    })

    test('应该生成正确的日志流 URL', () => {
      const url = mcpConfig.getMCPLogStreamUrl()
      expect(url).toBe('http://localhost:3001/api/v1/mcp/logs/stream')
    })
  })
  ...
  describe('配置更新', () => {
    ...
    test('应该能够部分更新配置', () => {
      ...
      expect(config.BACKEND_PORT).toBe('3001') // 保持默认值
    })
  ...
  describe('边界情况', () => {
    ...
    test('应该处理特殊字符的工具名称', () => {
      ...
      expect(url).toBe('http://localhost:3001/api/v1/mcp/tools/search-projects_v2')
    })
    ...
    test('应该处理 IPv6 地址', () => {
      ...
      const url = mcpConfig.getBackendBaseUrl()
      expect(url).toBe('http://::1:3001/api/v1')
    })
  })
})
...
describe('MCP 配置集成测试', () => {
  ...
  test('重置配置后所有 URL 都应该恢复默认值', () => {
    ...
    const urls = getMCPUrls()
    expect(urls.backend).toContain('localhost:3001')
