...
import { Folder, FolderTree, X, Info, Settings } from 'lucide-react';
import { toast } from 'react-hot-toast';
import { useAPIGroups } from '../../../../hooks/useAPIGroups';

interface CreateAPIGroupModalProps {
  isOpen: boolean;
  onClose: () => void;
  projectId: string;
  parentGroup?: any;
...
export const CreateAPIGroupModal: React.FC<CreateAPIGroupModalProps> = ({
  isOpen,
  onClose,
  projectId,
  parentGroup,
  ...
  const { createGroup } = useAPIGroups(projectId);
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      ...
      toast.success('API分组创建成功');
      reset();
      onClose();
      onSuccess?.();
    } catch (error) {
      console.error('创建API分组失败:', error);
      toast.error('创建API分组失败');
    } finally {
      setLoading(false);
    }
  };
  ...
  const availableParentGroups = groups.filter(group => 
      ...
      <div className="bg-bg-paper rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-hidden">
        ...
        <div className="flex items-center justify-between p-6 border-b border-border-primary">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-text-primary">创建API分组</h2>
          </div>
          <button
            onClick={onClose}
