...
export const APISyncPanel: React.FC<APISyncPanelProps> = ({ projectId }) => {
  ...
  const fetchSyncConfigurations = async () => {
    try {
      ...
      const response = await fetch(
        `/api/v1/api-management/sync-configurations?projectId=${projectId}`
      )
  ...
  const detectConflicts = async (configId?: string) => {
    try {
      ...
      const response = await fetch(
        `/api/v1/api-management/sync-configurations/${targetConfigId}/conflicts`
      )
      const data = await response.json()
  ...
  const executeSync = async (configId: string, dryRun: boolean = false) => {
    try {
      ...
      if (data.success) {
        ...
        if (dryRun) {
          toast.success(
            `预览完成: 将创建 ${result.createdEndpoints} 个端点，更新 ${result.updatedEndpoints} 个端点`
          )
        } else {
      ...
      <div className="bg-bg-paper rounded-lg border border-gray-200 shadow-sm p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          ...
          <div className="text-center">
            ...
            <div className="text-sm text-text-secondary">成功同步</div>
            <div className="text-xs text-gray-500 mt-1">
              今日:{' '}
              {
                syncHistory.filter(
          ...
          <div className="text-center">
            ...
            <div className="text-sm text-text-secondary">待解决冲突</div>
            <div className="text-xs text-gray-500 mt-1">需要注意</div>
          </div>
          <div className="text-center">
            <div className="text-3xl font-bold text-red-600">
              {syncHistory.filter(h => h.status === 'FAILED').length}
            </div>
            <div className="text-sm text-text-secondary">失败同步</div>
                    ...
                    >
                      <div className="flex items-start justify-between mb-4">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center space-x-4 text-sm text-gray-500">
                            <span>
                              同步方向:{' '}
                              {config.syncDirection === 'MODEL_TO_API'
                                ? '模型→API'
                                : config.syncDirection === 'API_TO_MODEL'
                                  ? 'API→模型'
                                  : '双向同步'}
                            </span>
                            <span>
                              ...
                              {config.conflictResolution === 'MANUAL'
                                ? '手动处理'
                                : config.conflictResolution === 'MODEL_WINS'
                                  ? '模型优先'
                                  : config.conflictResolution === 'API_WINS'
                                    ? 'API优先'
                                    : '自动合并'}
                            </span>
                            {config.lastSyncAt && (
                      ...
                      <div className="bg-bg-secondary rounded-lg p-4 text-sm">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                          ...
                          {config.nextSyncAt && (
                            <div>
                              <span className="text-text-secondary">下次同步:</span>
                              <span className="ml-1 font-medium">
                                {new Date(config.nextSyncAt).toLocaleString()}
                              </span>
                            </div>
                          )}
                          <div>
            ...
            {syncHistory.length === 0 ? (
              ...
              <div className="space-y-4">
                {syncHistory.map(history => (
                  <div key={history.id} className="border border-gray-200 rounded-lg p-6">
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center space-x-3">
                        ...
                        <div>
                          <h4 className="font-semibold text-text-primary">
                            同步执行 -{' '}
                            {syncConfigurations.find(c => c.id === history.configId)?.name ||
                              '未知配置'}
                          </h4>
                      ...
                      <div className="bg-bg-secondary rounded-lg p-4">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                          <div className="text-center">
                            ...
                            <div className="text-sm text-text-secondary">新建端点</div>
                          </div>
                          <div className="text-center">
                            <div className="text-2xl font-bold text-blue-600">
                              {history.result.updatedEndpoints}
                            </div>
                            <div className="text-sm text-text-secondary">更新端点</div>
                          </div>
                          <div className="text-center">
                            <div className="text-2xl font-bold text-red-600">
                              {history.result.deletedEndpoints}
                            </div>
                            <div className="text-sm text-text-secondary">删除端点</div>
                          </div>
                          <div className="text-center">
                            <div className="text-2xl font-bold text-orange-600">
                              {history.result.conflicts.length}
                            </div>
                            <div className="text-sm text-text-secondary">冲突数量</div>
                          </div>
                        </div>
                    ...
                    <div className="flex items-center justify-between mt-4 text-sm text-gray-500">
                      <span>
                        ...
                        {history.endTime
                          ? Math.round(
                              (new Date(history.endTime).getTime() -
                                new Date(history.startTime).getTime()) /
                                1000
                            )
                          : '计算中'}
                        秒
          ...
          <div className="px-6 py-4 border-b border-gray-200 flex justify-between items-center">
            ...
            <div className="flex items-center space-x-3">
              <select
                ...
                onChange={e =>
                  setSelectedConfig(syncConfigurations.find(c => c.id === e.target.value) || null)
                }
                className="px-3 py-2 border border-gray-300 bg-bg-secondary focus:outline-none rounded-md text-sm"
              >
                      ...
                      <div className="flex items-start space-x-3">
                        ...
                        <div>
                          <h4 className="font-medium text-orange-800">
                            {conflict.type === 'field_count_mismatch'
                              ? '字段数量不匹配'
                              : conflict.type === 'field_type_mismatch'
                                ? '字段类型不匹配'
                                : conflict.type === 'missing_parameter'
                                  ? '缺少API参数'
                                  : conflict.type === 'path_mismatch'
                                    ? '路径不匹配'
                                    : '其他冲突'}
                          </h4>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {conflict.tableId && (
                          <div>
                            ...
                            <span className="ml-2 text-text-secondary">{conflict.tableId}</span>
                          </div>
                        )}
                        {conflict.endpointId && (
                          <div>
                            <span className="font-medium text-text-primary">API端点ID:</span>
                            <span className="ml-2 text-text-secondary">{conflict.endpointId}</span>
                          </div>
                        )}
                      </div>
                    </div>
