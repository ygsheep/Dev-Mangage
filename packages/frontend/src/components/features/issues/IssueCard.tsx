...
export const IssueCard: React.FC<IssueCardProps> = ({ issue, onClick }) => {
  ...
  return (
    <div
      className="bg-bg-paper border border-gray-200 rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer"
      onClick={onClick}
    >
      {/* 头部：标题和 GitHub 链接 */}
      <div className="flex items-start justify-between mb-3">
        <div className="flex-1">
          <div className="flex items-center space-x-2 mb-1">
            <h3 className="text-lg font-medium text-text-primary hover:text-blue-600">
              {issue.title}
            </h3>
            {issue.githubNumber && (
              <span className="text-sm text-gray-500">#{issue.githubNumber}</span>
            )}
          </div>
        ...
        {issue.githubHtmlUrl && (
          <a
            ...
            className="ml-4 text-gray-400 hover:text-text-secondary"
      ...
      <div className="flex flex-wrap items-center gap-2 mb-3">
        {/* 状态标签 */}
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${ISSUE_STATUS_COLORS[issue.status]}`}
        >
        ...
        {/* 优先级标签 */}
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${ISSUE_PRIORITY_COLORS[issue.priority]}`}
        >
        ...
        {/* 类型标签 */}
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${ISSUE_TYPE_COLORS[issue.issueType]}`}
        >
        ...
        {/* 同步状态标签 */}
        <span
          className={`px-2 py-1 text-xs font-medium rounded-full ${SYNC_STATUS_COLORS[issue.syncStatus]}`}
        >
      ...
      <div className="flex items-center justify-between text-sm text-gray-500">
        <div className="flex items-center space-x-4">
          ...
          {issue.estimatedHours && (
            <div className="flex items-center space-x-1">
              ...
              {issue.actualHours && <span className="text-gray-400">/ {issue.actualHours}h</span>}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          {/* 最后更新时间 */}
          <span className="text-xs">{formatTimeAgo(issue.updatedAt)}</span>
        </div>
      </div>
