...
export const IssuesFilterBar: React.FC<IssuesFilterBarProps> = ({
  ...
  return (
    <div className="bg-bg-paper border border-gray-200 rounded-lg p-4 mb-6">
      {/* 搜索栏 */}
      <form onSubmit={handleSearchSubmit} className="mb-4">
        <div className="relative">
          <input
            ...
            className="w-full pl-10 pr-4 py-2 border border-gray-300 bg-bg-secondary focus:outline-none rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
          ...
          {searchInput && (
            ...
            >
              <svg
                className="h-4 w-4 text-gray-400 hover:text-text-secondary"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <select
            ...
            className="text-sm border border-gray-300 bg-bg-secondary focus:outline-none rounded px-2 py-1 focus:ring-1 focus:ring-blue-500"
          >
        ...
        {/* 优先级筛选 */}
        <div className="flex items-center space-x-2">
          <label className="text-sm font-medium text-text-secondary">优先级:</label>
          <select
        ...
        {/* 分配人筛选 */}
        <div className="flex items-center space-x-2">
          <label className="text-sm font-medium text-text-secondary">分配给:</label>
          <input
            type="text"
            placeholder="用户名"
            value={filters.assignee || ''}
            onChange={e => handleFilterChange('assignee', e.target.value)}
            className="text-sm border border-gray-300 bg-bg-secondary focus:outline-none rounded px-2 py-1 w-24 focus:ring-1 focus:ring-blue-500"
          />
        </div>
      ...
      <div className="flex flex-wrap items-center gap-2 mt-3 pt-3 border-t border-gray-100">
        <span className="text-sm text-text-secondary">快速筛选:</span>

        <button
          onClick={() =>
            onFiltersChange({ status: IssueStatus.OPEN, priority: IssuePriority.HIGH })
          }
        ...
        <button
          onClick={() =>
            onFiltersChange({ issueType: IssueType.FEATURE, status: IssueStatus.OPEN })
          }
