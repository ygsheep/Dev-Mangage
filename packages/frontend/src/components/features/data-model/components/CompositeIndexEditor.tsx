import { DatabaseIndex, DatabaseTable } from '@shared/types'
import {
  AlertTriangle,
  CheckCircle,
  Database,
...
const CompositeIndexEditor: React.FC<CompositeIndexEditorProps> = ({
  ...
  const validateIndex = useCallback(() => {
    ...
    if (indexType === 'FULLTEXT') {
      const nonTextFields = indexFields.filter(
        f => !['VARCHAR', 'TEXT', 'LONGTEXT'].includes(f.fieldType.toUpperCase())
      )
    ...
    const duplicateIndex = existingIndexes.find(
      idx =>
  ...
  const updateField = (index: number, updates: Partial<IndexField>) => {
    setIndexFields(prev => prev.map((field, i) => (i === index ? { ...field, ...updates } : field)))
  }
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-bg-paper rounded-lg shadow-xl w-full max-w-4xl h-[90vh] flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-gradient-header">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-text-secondary">表: {table.displayName || table.name}</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-hidden flex">
          {/* 左侧配置面板 */}
          <div className="w-1/3 border-r border-gray-200 p-6 overflow-y-auto custom-scrollbar">
                    ...
                    return (
                      <div
                        ...
                        className={`p-3 border rounded-lg cursor-pointer transition-colors ${
                          isAdded
                            ? 'border-gray-300 bg-bg-secondary bg-bg-secondary cursor-not-allowed'
              ...
              <div>
                <div className="flex items-center justify-between mb-4">
                  ...
                  {indexFields.length > 0 && (
                    <div className="text-sm text-text-secondary">拖拽调整字段顺序</div>
                  )}
                </div>

                {indexFields.length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-gray-300 bg-bg-secondary rounded-lg">
                    <Database className="w-16 h-16 text-gray-300 mx-auto mb-4" />
                    <h4 className="text-lg font-medium text-text-primary mb-2">暂无索引字段</h4>
                    <p className="text-text-secondary">从左侧选择字段添加到索引中</p>
                  </div>
                ) : (
                            ...
                            <div className="flex items-center space-x-4">
                              <div className="flex items-center space-x-2">
                                ...
                                <select
                                  value={field.order}
                                  onChange={e =>
                                    updateField(index, { order: e.target.value as 'ASC' | 'DESC' })
                                  }
                                  className="text-sm border border-gray-300 bg-bg-secondary focus:outline-none rounded px-2 py-1"
                                >
                              ...
                              {field.fieldType.includes('VARCHAR') && (
                                <div className="flex items-center space-x-2">
                                  ...
                                  <input
                                    ...
                                    className="text-sm border border-gray-300 bg-bg-secondary focus:outline-none rounded px-2 py-1 w-16"
                                    placeholder="可选"
                                  />
                                </div>
                              )}
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-bg-secondary">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              <button onClick={handleReset} className="btn-outline flex items-center space-x-2">
