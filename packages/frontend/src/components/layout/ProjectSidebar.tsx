import { Brain, Bug, Code2, Database, GitBranch, Home, Layers, Settings } from 'lucide-react'
...
const ProjectSidebar: React.FC<ProjectSidebarProps> = ({ projectName, isCollapsed }) => {
      ...
      <div className="px-3 space-y-1">
        {projectNavigation.map(item => {
          ...
          return (
            ...
            >
              <Icon
                className={`flex-shrink-0 ${isCollapsed ? 'w-4 h-4' : 'w-3 h-3 mr-3'} ${
              ...
              {/* 激活状态指示器 */}
              {active && <div className="w-1 h-1 bg-white rounded-full ml-2 flex-shrink-0" />}
            </Link>
          )
        })}
      </div>
