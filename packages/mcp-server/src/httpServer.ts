...
class HTTPMCPServer {
  ...
  private async searchAPIs(args: any) {
    ...
    // 按HTTP方法过滤
    if (method) {
      results = results.filter(result => result.item.method.toLowerCase() === method.toLowerCase())
    }
      ...
      for (const fResult of fuzzyResults) {
        ...
        if (existing) {
          ...
          existing.combinedScore =
            existing.vectorScore * vectorWeight + fResult.fuzzyScore * fuzzyWeight
...
if (require.main === module) {
  const port = parseInt(process.env.HTTP_MCP_PORT || '3000')
